...
class WebDriver(Scraper):
    def __init__(self, url=None, id=None):
        ...
        if id is None:
            self.session = webdriver.Chrome(chrome_options=options, executable_path='driver/chromedriver')
    ...
    def findElementByName(self, name):
        return self.session.find_element(By.NAME, name)
    ...
    def get_browser_log(self):
        browser_log = self.session.get_log('performance')
...
class Rest(Scraper):
    ...
    def setMethod(self, method):
        if type(method) is not str and (
                method != 'GET' or method != 'POST' or method != 'GET_JSON' or method != 'POST_JSON'):
